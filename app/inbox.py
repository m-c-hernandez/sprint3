...
def getDB():
    return send_file(current_app.config['DATABASE'], as_attachment=True)
        #SELECT user.username, message.created, message.subject, message.body FROM message INNER JOIN user ON user.id = message.from_id WHERE message.to_id= ? ORDER by created DESC
